...
[DynamicWebApi]
public abstract class MongoRestServiceBase<T> : IDisposable, IDynamicWebApi, IAsyncRestService<T> where T : class, IDataModel
{
    ...
    public MongoRestServiceBase(IDatabaseSetting settings) => _collection = MongoDbQueryExtensions.GetCollection<T>(settings, $"{typeof(T).Name.ToCamelCase()}s");
